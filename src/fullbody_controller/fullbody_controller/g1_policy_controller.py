...
"""G1-only ROS 2 controller that runs the trained policy in Isaac Sim.

This is the **pre-refactor** controller, kept as a known-good reference for the G1
humanoid locomotion policy specifically. It hardcodes the canonical G1 obs assembly
(``lin_vel | ang_vel | gravity | cmd_vel | joint_pos_rel | joint_vel | last_action``)
in ``_compute_observation``; it cannot be used with policies whose observation layout
differs from G1's.

For the robot-agnostic version, see ``policy_controller.py`` -- that one drives the
obs assembly from ``IO_descriptors.yaml`` via OBS_PRODUCERS and is what new robots
(Go2, future) should use.
...
class G1FullbodyController(Node):
    """G1-specific policy controller (hardcoded obs layout, kept for reference).

    Subscribes to robot state, publishes joint position targets. See the module
    docstring for the distinction vs ``policy_controller.FullbodyController``.
    """

    def __init__(self):
        """Initialize the G1FullbodyController node."""
