...
"""Launch the policy controller for the Unitree Go2 quadruped.

Thin wrapper around ``policy_controller.launch.py`` that points ``policy_path``
at the Go2 checkpoint shipped under ``policy/go2_locomotion/`` in this package.
Pass through any other launch args by appending them on the command line,
e.g. ``ros2 launch fullbody_controller go2.launch.py decimation:=2``.
"""
...
def generate_launch_description():
    ...
    policy_path = os.path.join(share, 'policy', 'go2_locomotion', 'policy.pt')
